    ...
    if (entries.length === 0) {
        ...
        entries.forEach(entry => {
            const cell = createElement('div', 'ios-list-cell ios-list-cell-full', {
                onClick: () => Router.navigate('/global-settings/edit/' + entry.id)
            });
...
export default {
    ...
    icon: 'settings',
    routes: [
        { path: '/global-settings', render: renderGlobalSettings }
    ]
};
