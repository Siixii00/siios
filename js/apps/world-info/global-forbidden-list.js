...
async function renderGlobalForbidden() {
    ...
    if (entries.length === 0) {
        ...
        entries.forEach(entry => {
            const cell = createElement('div', 'ios-list-cell ios-list-cell-full', {
                onClick: () => Router.navigate('/global-forbidden/edit/' + entry.id)
            });
...
export default {
    ...
    icon: 'block',
    routes: [
        { path: '/global-forbidden', render: renderGlobalForbidden }
    ]
};
