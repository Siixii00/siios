...
- 體型尊重：必須嚴格閱讀並遵守使用者(user)的外觀描述與世界書中的身材條目。禁止使用鎖骨、腰窩、膝彎、腰線、曲線、A4腰、漫畫腿、竹竿、紙片人、骨瘦如柴、瘦得皮包骨等不健康或不真實的體型詞彙。若使用者設定為較為圓潤、豐滿、健康體型，必須完全遵循該設定，絕不可使用上述詞彙。` : (environ.isPublic ? '- 這是公開頻道，內容會被其他成員看到。' : '- 這是私訊／群組對話。')}
